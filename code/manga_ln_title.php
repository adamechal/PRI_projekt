...
while ($row = $result->fetch_assoc()) {
    if (slugify($row['en_name']) === $slug && in_array(strtolower($row['type_name']), ['manga', 'ln'])) {
        $foundTitle = $row;
        break;
    }
}
...
if (!$foundTitle) {
    ...
    exit("Manga nebo LN verze titulu nebyla nalezena.");
}

$dom = new DOMDocument('1.0', 'UTF-8');
$dom->formatOutput = true;
$dom->appendChild($dom->createProcessingInstruction('xml-stylesheet', 'type="text/xsl" href="manga_ln_detail.xsl"'));
...
// Druhý průchod pro nezobrazené MANGA/LN bez vztahu
$res = $mysqli->query("
    SELECT t.*, ty.type_name
    FROM titles t
    JOIN types ty ON t.id_type = ty.id_type
    WHERE t.id_series = $seriesId
